...
async function importGame(game) {
  ...
  for (const group of groupsToProcess) {
    ...
    // Insert price snapshots for cards we successfully upserted
    const priceRows = insertedCards
      .map((card) => {
        const price = priceByProductId.get(Number(card.tcgcsv_product_id));
        if (!price || price.marketPrice == null) return null;
        return {
          card_id: card.id,
